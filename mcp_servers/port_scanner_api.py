...
class ScanRequest(BaseModel):
    ...
    scan_type: str = Field("tcp", description="Scan type: tcp, syn, udp, or both")
...
@app.post("/scan/syn")
async def syn_scan(target: str, ports: str = "1-1000"):
    """Perform SYN scan (requires privileges)"""
    try:
        request = ScanRequest(
            target=target,
            ports=ports,
        ...
        print(f"[ERROR] SYN scan failed for {target}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"SYN scan failed: {str(e)}")
...
async def comprehensive_scan(target: str):
    """Comprehensive port scan with all options"""
    request = ScanRequest(
        target=target,
        ports="1-1000",  # Reduced from 1-65535 to prevent timeouts
        scan_type="tcp",
        timing=3,
        service_detection=True,
        script_scan=["banner", "version"]
    )
    result = await scanner.scan_target(
        request.target, 
        request.ports,
